...
| `OIDC_RSA_PRIVATE_KEY` | — | RSA private key (PEM format) that enables OpenID Connect on the built-in OAuth2 provider and signs its ID tokens. Not needed for ordinary OAuth2 access-token issuance, unless an application signs tokens with `RS256`. Setting it also adds the `openid`/`profile` scopes. |
