...
| `RATE_LIMIT_WEBHOOK` | `3000/5m` | Request limit for the `webhook` scope (inbound channel deliveries: Telegram, Twilio, Meta Cloud API, Turn, SureAdhere, CommCare Connect, Slack events). Keyed by the channel identifier the webhook URL carries (Telegram's `channel_external_id`, Turn's `experiment_id`, SureAdhere's `sureadhere_tenant_id`), then by client IP for webhooks whose URL carries no identifier. Set `RATE_LIMIT_TRUSTED_PROXY_COUNT` before enforcing behind a proxy, or those IP-keyed callers all share one bucket. Fails open: if the limiter's cache is unreachable, requests are served. |
