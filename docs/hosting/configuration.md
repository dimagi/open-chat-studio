...
| `RATE_LIMIT_CHANNELS` | `3000/5m` | Request limit for the `channels` scope (inbound channel deliveries: Telegram, Twilio, Meta Cloud API, Turn, SureAdhere, CommCare Connect, Slack events). Keyed by the channel identifier the webhook URL carries (Telegram's `channel_external_id`, Turn's `experiment_id`, SureAdhere's `sureadhere_tenant_id`), then by client IP for webhooks whose URL carries no identifier. Set `RATE_LIMIT_TRUSTED_PROXY_COUNT` before enforcing behind a proxy, or those IP-keyed callers all share one bucket. Fails open: if the limiter's cache is unreachable, requests are served. |
| `RATE_LIMIT_CREDENTIALS` | `100/5m` | Request limit for the `credentials` scope (the OAuth client-credential endpoints at `/o/token/`, `/o/revoke_token/` and `/o/introspect/`, and API requests whose key or bearer token is rejected). Keyed by client IP, because a caller failing authentication has no identity to key on. Set `RATE_LIMIT_TRUSTED_PROXY_COUNT` before enforcing behind a proxy, or every caller shares one bucket. The one scope that fails closed: where the others serve the request when the limiter's cache is unreachable, this one refuses it once enforcement is on, so that a counter nobody can read does not become a way to brute force credentials unobserved. Successful API requests are counted under `RATE_LIMIT_API` instead, so a working integration is never charged to this scope. |
