# ADR-0039: Stateless signed session tokens with server-side inactivity expiry
...
- **No expiry (token valid for session lifetime)** — rejected: preserves indefinite access for a leaked token, the problem ADR-0038 set out to bound.
