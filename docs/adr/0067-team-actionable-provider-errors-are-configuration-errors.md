...
- Translation happens at two boundaries: the node one, in `PipelineNode.process` and the router function, and again under `with_llm_retry` for runnables invoked outside a node. Both sit *inside* their retry wrapper, which is what makes the split work: a transient fault is never translated, so the retry still sees its native type and backs off as before, while a translated one is refused by `should_retry_exception` and fails on the first attempt instead of burning the full backoff.
