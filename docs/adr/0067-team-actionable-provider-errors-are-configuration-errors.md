...
The exceptions this produces are the loudest class of unresolved issue in the project, and they share a shape: the provider rejects the call, nobody at Dimagi can fix it, and no retry will ever succeed. An OpenAI account with no credits accounted for 469 events from a single team; a withdrawn Gemini model, an Anthropic usage cap, a mistyped key and a conversation past the context window make up most of the rest. Each failed its Celery task and paged us about a team's billing.
...
- The test is who holds the fix. A team member can add credit, replace a key, pick a different model or shorten the history; a participant cannot, and neither can we. Transient faults — rate limits, overload, connection errors — fail this test and keep their native SDK type.
...
- Misclassification hides a genuine fault behind canned text. The risk is concentrated in the message-text matching Anthropic forces, which will drift when Anthropic rewords a billing error; the team then sees a generic failure instead of a specific one, but is still notified.
