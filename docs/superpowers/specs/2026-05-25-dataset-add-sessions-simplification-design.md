...
| `apps/evaluations/tables.py` | Remove the `clone_filtered_only` column from `EvaluationSessionsSelectionTable`. On the remaining `selection` column, set `verbose_name=""` so the header shows just the bare select-all checkbox (no "All" label, no help tooltip — the column's purpose is obvious in the new layout). |
| `assets/javascript/apps/human_annotations/session-selector.js` | Add a `hasActiveFilters` getter: returns `true` if any param in `filterParams` has a name starting with `filter_` (matching the existing convention in `CreateDataset.get_initial`). Keep all other behavior intact. The Alpine watcher that resets `messageScope` lives in the page's `add_sessions.html` script block, not in the shared component, to keep the shared component dataset-agnostic. |
