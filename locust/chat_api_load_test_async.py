...
class ChatUser(HttpUser):
    ...
    def start_session(self) -> bool:
        ...
        with self.client.post(
            "/api/chat/a/start/", json=payload, headers=self.get_headers(), catch_response=True, name="Start Session"
        ) as response:
            if response.status_code == 201:
                ...
                logger.error(f"Failed to start session: {response.status_code}: {payload}")
    ...
    def send_message(self, message_text: str) -> str | None:
        ...
        with self.client.post(
            f"/api/chat/a/{self.session_id}/message/",
            json=payload,
            ...
            if response.status_code == 200:
                ...
                logger.debug(f"Message sent, response: {message}")
                response.success()
                return message
            else:
    ...
    def have_conversation(self):
        ...
        response_text = self.send_message(message_text)
