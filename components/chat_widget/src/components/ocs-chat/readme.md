...
| `disabled`                   | `disabled`                      | Render the widget in a read-only state. When `true`, the chat history stays visible and scrollable but the message input and send controls are hidden, and message sending is blocked at the source. Useful for maintenance windows, expired sessions, scheduled downtime or post-session review. | `boolean`                 | `false`                            |
