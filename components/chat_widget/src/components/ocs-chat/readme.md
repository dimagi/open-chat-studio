...
| `persistentSession`          | `persistent-session`            | Where to keep the session so a conversation can resume after a page reload. - `true` (default): `localStorage`, survives tab close. - `"tab"`: `sessionStorage`, survives reload, cleared when the tab closes. - `false`: nothing is stored; a reload starts over. Ignored when `sessionId` is provided.           | `"false" \| "tab" \| "true" \| boolean`       | `true`                             |
