...
urlpatterns = [
    ...
    # The export-specific info.description is applied by the ``set_export_description`` postprocessing
    # hook (keyed on api_version) rather than ``custom_settings`` here, so the committed
    # ``api-schemas/export.yml`` -- generated by the ``spectacular`` command, which ignores
    # ``custom_settings`` -- carries the same description this served schema does.
    path("api/export/schema/", SpectacularAPIView.as_view(api_version="export"), name="schema-export"),
