...
@login_and_team_required
@permission_required("ocs_channels.add_experimentchannel", raise_exception=True)
...
def update_delete_channel(request, team_slug: str, experiment_id: int, channel_id: int):
    ...
    if request.POST.get("action") == "delete":
        if not request.user.has_perm("ocs_channels.delete_experimentchannel"):
            raise PermissionDenied

        channel.soft_delete()
        return redirect("experiments:single_experiment_home", team_slug, experiment_id)

    if not request.user.has_perm("ocs_channels.change_experimentchannel"):
        raise PermissionDenied
