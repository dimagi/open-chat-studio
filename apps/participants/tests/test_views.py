...
@pytest.mark.django_db()
def test_single_participant_home_with_experiment_renders_session_table_placeholder(client, team_with_users):
    """The sessions table loads via a separate htmx fragment request (ParticipantSessionsTableView),
    so this page only needs the placeholder, the correct hx-get URL, and the Continue Chat widget
    launcher the table's buttons depend on."""
    participant = ParticipantFactory.create(team=team_with_users)
    ...
    assert "ocsContinueSessionChat = function" in content

    session_table_url = reverse(
    ...
    assert f'hx-get="{session_table_url}"' in content


@pytest.mark.django_db()
def test_participant_sessions_table_view_scopes_to_the_participant(client, team_with_users):
    """Two participants on the same chatbot shouldn't see each other's sessions in this table."""
    ...
    url = reverse(
        "participants:participant-sessions-table",
        kwargs={"team_slug": team.slug, "participant_id": participant.id, "experiment_id": experiment.id},
    )
    response = client.get(url)
...
@pytest.mark.django_db()
def test_participant_sessions_table_view_paginates(client, team_with_users):
    """Real pagination: more than one page's worth of sessions must not all render at once."""
    team = team_with_users
    experiment = ExperimentFactory.create(team=team)
    participant = ParticipantFactory.create(team=team)
    sessions = [
        ExperimentSessionFactory.create(team=team, experiment=experiment, participant=participant) for _ in range(15)
    ]
    ...
    assert first_page.context_data["table"].paginator.num_pages == 2
    assert len(first_page.context_data["table"].page.object_list) == 10
    assert len(second_page.context_data["table"].page.object_list) == 5
    first_page_ids = {row.record.id for row in first_page.context_data["table"].page.object_list}
    second_page_ids = {row.record.id for row in second_page.context_data["table"].page.object_list}
    assert first_page_ids | second_page_ids == {s.id for s in sessions}
