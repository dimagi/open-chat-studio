...
@pytest.mark.django_db()
def test_llm_provider_edit_view_shows_the_status_badge_and_verify_button(team_with_users, authed_client):
    """Verification is a status you read in the header, not an action whose result you have
    to catch. A provider nobody has verified says so rather than showing nothing."""
    ...
    assert "These credentials have never been verified" in content
...
@pytest.mark.django_db()
def test_non_llm_provider_edit_view_has_no_status_badge(team_with_users, authed_client):
    """Only LLM providers have credentials to verify, so only they get the status header."""
...
class TestSavingVerifiesCredentials:
    """Saving credentials verifies them, and records the result - so the badge on the page
    the user lands on already reflects the save they just made. Saving is the only thing
    that verifies, so every classification has to survive this path."""
    ...
    def test_changed_credentials_are_tested_and_the_result_recorded(self, team_with_users, authed_client):
        provider = _testable_llm_provider(team_with_users)

        with mock.patch.object(LlmProvider, "invoke_test_model") as invoked:
            authed_client.post(
    ...
    def test_an_unrelated_edit_does_not_spend_an_external_call(self, team_with_users, authed_client):
        """Renaming is not a credential change, so there is nothing new to verify."""
        provider = _testable_llm_provider(team_with_users)
        api_key = provider.config["openai_api_key"]
    ...
    def test_a_passing_test_returns_to_the_team_list(self, team_with_users, authed_client):
    ...
    def test_a_failing_test_lands_on_the_page_that_says_why(self, team_with_users, authed_client):
        """Regression: the save must redirect back to this same edit page, not the team list -
        the badge and the failure panel that explain the result only render here."""
        provider = _testable_llm_provider(team_with_users)
    ...
    def test_a_rate_limited_test_also_lands_on_the_edit_page(self, team_with_users, authed_client):
        """The credentials are probably fine, but the user still has something to do about
        it - retry - and the Test connection button is on this page."""
        provider = _testable_llm_provider(team_with_users)
        url = self._edit_url(team_with_users, provider)

        with mock.patch.object(LlmProvider, "invoke_test_model", side_effect=_FakeTransientError("rate limited")):
            response = authed_client.post(url, data={"name": provider.name, "openai_api_key": "a-new-key"}, follow=True)

        assert response.redirect_chain == [(url, 302)]
        assert "Couldn&#x27;t verify" in response.content.decode()

    def test_an_untestable_provider_type_is_not_detoured(self, team_with_users, authed_client):
        """Voyage AI can never pass this test, so sending the user to the edit page every
    ...
    def test_a_failure_keeps_the_provider_response(self, team_with_users, authed_client):
        """The raw response stays one click away for whoever has to debug it."""
        provider = _testable_llm_provider(team_with_users)
        exc = Exception("Incorrect API key provided: sk-p***lt")
        exc.status_code = 401

        with mock.patch.object(LlmProvider, "invoke_test_model", side_effect=exc):
            response = authed_client.post(
                self._edit_url(team_with_users, provider),
                data={"name": provider.name, "openai_api_key": "a-bad-key"},
                follow=True,
            )

        content = response.content.decode()
        assert "Show provider response" in content
        assert "sk-p***lt" in content

    def test_a_timeout_is_amber_not_red(self, team_with_users, authed_client):
        """A provider-SDK timeout carries no status code, so only the explicit timeout check
        keeps it out of the red bucket."""
        provider = _testable_llm_provider(team_with_users)

        with mock.patch.object(LlmProvider, "invoke_test_model", side_effect=_openai_timeout_error()):
            response = authed_client.post(
                self._edit_url(team_with_users, provider),
                data={"name": provider.name, "openai_api_key": "a-new-key"},
                follow=True,
            )
    ...
    def test_an_invalid_configuration_is_a_failure(self, team_with_users, authed_client):
        provider = _testable_llm_provider(team_with_users)
        error = ServiceProviderConfigError(provider.type, "invalid base_url")

        with mock.patch.object(LlmProvider, "invoke_test_model", side_effect=error):
            response = authed_client.post(
                self._edit_url(team_with_users, provider),
                data={"name": provider.name, "openai_api_key": "a-new-key"},
                follow=True,
        ...
        content = response.content.decode()
        assert "Verification failed" in content
        assert "The saved configuration is incomplete" in content

    def test_no_configured_model_points_at_the_models_tab(self, team_with_users, authed_client):
        ...
        content = response.content.decode()
        assert "Can&#x27;t verify" in content
        assert "Add a model on the Models tab first" in content

    def test_a_pass_shows_as_verified_on_the_next_visit(self, team_with_users, authed_client):
        """A pass sends the user to the team list, so the badge has to still be right when
        they come back to the provider."""
        provider = _testable_llm_provider(team_with_users)
        with mock.patch.object(LlmProvider, "invoke_test_model"):
            authed_client.post(
                self._edit_url(team_with_users, provider),
                data={"name": provider.name, "openai_api_key": "a-good-key"},
            )

        response = authed_client.get(self._edit_url(team_with_users, provider))

        assert b"Credentials verified" in response.content

    def test_a_save_never_fails_because_the_provider_did(self, team_with_users, authed_client):
        """The test runs after the save transaction commits, so a provider outage cannot
        cost the user the credentials they just entered."""
        provider = _testable_llm_provider(team_with_users)

        with mock.patch.object(LlmProvider, "invoke_test_model", side_effect=RuntimeError("boom")):
            authed_client.post(
                self._edit_url(team_with_users, provider),
        ...
        assert provider.config["openai_api_key"] == "a-new-key"
