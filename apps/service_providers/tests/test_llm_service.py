...
def test_open_ai_service():
    ...
    service = LlmProviderTypes.openai.get_llm_service(UsageRecorder(None), {"openai_api_key": "test"})
...
def test_anthropic_service():
    service = AnthropicLlmService(
        anthropic_api_key="test", anthropic_api_base="https://api.anthropic.com", usage_recorder=UsageRecorder(None)
    )
    assert not service.supports_transcription
