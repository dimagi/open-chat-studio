...
class PipelineStartAction(EventActionHandlerBase):
    def invoke(self, session: ExperimentSession, action) -> str:
        from apps.pipelines.models import (
            Pipeline,  # noqa: PLC0415  # Circular: pipelines.models imports events.models which imports events.actions
        )
        ...
        with trace_service.trace(
            ...
            from apps.chat.bots import (
                PipelineBot,  # noqa: PLC0415  # Circular: chat.bots imports events.models which imports events.actions
            )
