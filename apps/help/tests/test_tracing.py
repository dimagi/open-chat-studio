...
class TestGetHelpAgentTracer:
    @override_settings(OCS_LANGFUSE_PUBLIC_KEY="", OCS_LANGFUSE_SECRET_KEY="")
    def test_returns_none_when_unconfigured(self):
        assert get_help_agent_tracer() is None

    @override_settings(OCS_LANGFUSE_PUBLIC_KEY="pk", OCS_LANGFUSE_SECRET_KEY="", OCS_LANGFUSE_HOST="https://x")
    def test_returns_none_when_only_public_key_set(self):
        assert get_help_agent_tracer() is None

    @override_settings(OCS_LANGFUSE_PUBLIC_KEY="", OCS_LANGFUSE_SECRET_KEY="sk", OCS_LANGFUSE_HOST="https://x")
    def test_returns_none_when_only_secret_key_set(self):
        assert get_help_agent_tracer() is None

    @override_settings(
        OCS_LANGFUSE_PUBLIC_KEY="pk",
        OCS_LANGFUSE_SECRET_KEY="sk",
        OCS_LANGFUSE_HOST="https://x",
        OCS_LANGFUSE_SAMPLE_RATE=0.0,
    )
