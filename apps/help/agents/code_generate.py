...
def _get_system_prompt():
    return (Path(__file__).parent.parent / "code_generate_system_prompt.md").read_text()
...
class CodeGenerateAgent(BaseHelpAgent[CodeGenerateInput, CodeGenerateOutput]):
    ...
    def run(self) -> CodeGenerateOutput:
        from apps.pipelines.nodes.nodes import DEFAULT_FUNCTION  # noqa: PLC0415 - lazy import for startup performance
    ...
    def _run_with_retry(self, current_code: str, error: str | None, iteration: int) -> CodeGenerateOutput:
        ...
        from apps.pipelines.nodes.nodes import CodeNode  # noqa: PLC0415 - lazy import for startup performance
