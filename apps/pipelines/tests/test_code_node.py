...
def test_render_template_with_context_keys(pipeline, experiment_session):
    ...
    mock_participant = mock.Mock()
    mock_participant.identifier = "Participant123"
    mock_experiment_session = mock.Mock(participant=mock_participant)
    state = PipelineState(
        input="UserInput",
        temp_state={"my_key": "TempStateValue"},
        experiment_session=mock_experiment_session,
        messages=[],
    )
    result = create_runnable(pipeline, nodes).invoke(state)
    assert result["messages"], "No messages were generated by the pipeline."
    expected_output = (
        "input: UserInput\n" "temp_state.my_key: TempStateValue\n" "participant_id: Participant123\n"
    ).strip()
    assert result["messages"][-1].strip() == expected_output
