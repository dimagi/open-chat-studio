...
class TestHistoryMixin:
    def test_get_history_uses_session_history_for_global_type(self, history_node_factory):
        ...
        result = node.get_history()

        assert result == ["session-history"]
        mock_repo.get_session_messages.assert_called_once_with(node.get_history_mode(), exclude_message_id=None)
    ...
    def test_save_history_delegates_to_repo(self, history_node_factory):
        ...
        node.save_history("hello", "hi there")
    ...
    def test_save_history_skips_when_global(self, history_node_factory):
        ...
        result = node.save_history("hello", "hi there")
...
class TestLLMResponseWithPromptValidation:
    ...
    def test_openai_remote_vectorstore_limit_with_2_collections(self):
        ...
        openai_provider = LlmProviderFactory.create(type=LlmProviderTypes.openai.value.slug)

        # Create 2 remote collections with the same provider
        collection1 = CollectionFactory.create(is_remote_index=True, llm_provider=openai_provider, is_index=True)
        collection2 = CollectionFactory.create(is_remote_index=True, llm_provider=openai_provider, is_index=True)
    ...
    def test_openai_remote_vectorstore_limit_with_3_collections(self):
        ...
        collection1 = CollectionFactory.create(is_remote_index=True, llm_provider=openai_provider, is_index=True)
        collection2 = CollectionFactory.create(is_remote_index=True, llm_provider=openai_provider, is_index=True)
        collection3 = CollectionFactory.create(is_remote_index=True, llm_provider=openai_provider, is_index=True)
    ...
    def test_non_openai_provider_allows_more_than_2_remote_vectorstores(self):
        ...
        anthropic_provider = LlmProviderFactory.create(type=LlmProviderTypes.anthropic.value.slug)

        # Create 3 remote collections with the same provider
        collection1 = CollectionFactory.create(is_remote_index=True, llm_provider=anthropic_provider, is_index=True)
        collection2 = CollectionFactory.create(is_remote_index=True, llm_provider=anthropic_provider, is_index=True)
        collection3 = CollectionFactory.create(is_remote_index=True, llm_provider=anthropic_provider, is_index=True)
    ...
    def test_openai_local_vectorstores_not_limited(self):
        ...
        # Create OpenAI provider
        openai_provider = LlmProviderFactory.create(type=LlmProviderTypes.openai.value.slug)
        embedding_model = EmbeddingProviderModelFactory.create()
    ...
    def test_remote_vectorstores_must_have_same_llm_provider(self):
        openai_provider = LlmProviderFactory.create(type=LlmProviderTypes.openai)
        anthropic_provider = LlmProviderFactory.create(type=LlmProviderTypes.anthropic)
    ...
    def test_local_vectorstores_can_have_different_llm_provider(self):
        ...
        collection1 = CollectionFactory.create(is_remote_index=False, llm_provider=openai_provider, is_index=True)
    ...
    def test_local_vectorstores_must_have_summary_when_more_than_one(self):
        ...
        collection1 = CollectionFactory.create(is_remote_index=False, llm_provider=openai_provider, is_index=True)
        collection2 = CollectionFactory.create(is_remote_index=False, llm_provider=openai_provider, is_index=True)
