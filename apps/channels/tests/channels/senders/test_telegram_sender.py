...
from apps.channels.telegram_channel import TelegramSender
...
class TestSendText:
    def test_sends_text_message(self, sender, telebot):
        with patch("apps.channels.telegram_channel.antiflood") as mock_antiflood:
    ...
    def test_propagates_send_errors(self, sender):
        with patch("apps.channels.telegram_channel.antiflood", side_effect=RuntimeError("boom")):
