...
@patch("apps.channels.channels_v2.slack_channel.SlackChannel.messaging_service", new_callable=mock.PropertyMock)
def test_ad_hoc_bot_message(messaging_service, get_user_message, slack_channel):
    ...
    messaging_service().send_text_message.assert_called_once_with(
        "Hi",
        from_="",
        to="channel_id",
...
def test_send_message_to_user_with_file(slack_channel, slack_service):
    ...
    mock_service = Mock(spec=SlackService)
    ...
    # Text message sent via messaging service
    mock_service.send_text_message.assert_called_once()
    # File sent via messaging service
    mock_service.send_file_message.assert_called_once()
