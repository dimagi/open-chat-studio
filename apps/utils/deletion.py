...
def delete_object_with_auditing_of_related_objects(obj):
    ...
    """
    from field_audit.models import (  # noqa: PLC0415  # avoid AppRegistryNotReady (deletion.py is imported in files/apps.py)
        AuditAction,
        AuditingManager,
    )
...
def _queryset_update_with_auditing(queryset, **kw):
    ...
    """
    from field_audit import (  # noqa: PLC0415  # avoid AppRegistryNotReady (deletion.py is imported in files/apps.py)
        AuditService,
    )
    from field_audit.models import (  # noqa: PLC0415  # avoid AppRegistryNotReady (deletion.py is imported in files/apps.py)
        AuditEvent,
...
def get_related_objects(instance, pipeline_param_key: str | None = None) -> list:
    from apps.pipelines.models import (  # noqa: PLC0415  # avoid AppRegistryNotReady (deletion.py is imported in files/apps.py)
        Node,
    )
...
def _get_related_pipeline_experiments_queryset(
    ...
    from apps.experiments.models import (  # noqa: PLC0415  # avoid AppRegistryNotReady (deletion.py is imported in files/apps.py)
        Experiment,
    )
...
def get_admin_emails_with_delete_permission(team):
    from apps.teams.models import (  # noqa: PLC0415  # avoid AppRegistryNotReady (deletion.py is imported in files/apps.py)
        Membership,
    )
