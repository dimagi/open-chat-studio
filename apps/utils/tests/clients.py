...
class ApiTestClient(APIClient):
    def __init__(self, user, team, auth_method="api_key", read_only=False, scopes=None):
        ...
        scope = " ".join(scopes if scopes is not None else settings.OAUTH2_PROVIDER["SCOPES"])
        ...
        elif auth_method == "oauth_client_credentials":
            # Machine token: pinned to a team via the Application, with no user.
    ...
    def request(self, *args, **kwargs):
        ...
        elif self.auth_method in ("oauth", "oauth_client_credentials"):
