...
def test_triggering_a_config_with_only_archived_evaluators_is_refused(url_name, client, team_with_users):
    """Both trigger views redirect to the runs home with an error message when only evaluator is archived."""
    ...
    assert response.url == reverse("evaluations:evaluation_runs_home", args=[team_with_users.slug, config.id])
    error_messages = [str(m) for m in get_messages(response.wsgi_request)]
    assert any(config.name in message for message in error_messages)
...
def test_auto_run_skips_a_config_with_only_archived_evaluators(team_with_users):
    """A delta run is not triggered for a config whose only evaluator is archived."""
    evaluator = EvaluatorFactory.create(team=team_with_users)
