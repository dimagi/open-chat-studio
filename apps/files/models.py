...
class FileChunkEmbedding(BaseTeamModel, VersionsMixin):
    ...
    # Lexical half of hybrid search, written by the indexing pipeline rather than generated by
    # Postgres. A generated column would have to hard-code one text search configuration in its
    # DDL, and `to_tsvector(<column>::regconfig, ...)` is rejected as not immutable, so a
    # generated column cannot vary the configuration per row. Collections choose their own
