...
urlpatterns = [
    ...
    # Shadows the upstream `token`, `revoke_token` and `introspect` routes so every
    # endpoint that authenticates a client_id/client_secret pair is counted against
    # the credentials scope. Declared before the include so these patterns match
    # first; each keeps its upstream name, so reverse() is unaffected.
    path("o/token/", rate_limited("credentials")(oauth2_views.TokenView.as_view()), name="token"),
    path(
        "o/revoke_token/",
        rate_limited("credentials")(oauth2_views.RevokeTokenView.as_view()),
        name="revoke-token",
    ),
    path(
        "o/introspect/",
        rate_limited("credentials")(oauth2_views.IntrospectTokenView.as_view()),
        name="introspect",
    ),
