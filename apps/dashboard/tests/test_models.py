...
class TestDashboardMetricsSnapshot:
    ...
    def test_snapshot_generation(self, team, experiment, participant, experiment_session, chat, user):
        ...
        ChatMessage.objects.create(chat=chat, message_type=ChatMessageType.HUMAN, content="Test human message")
        ChatMessage.objects.create(chat=chat, message_type=ChatMessageType.AI, content="Test AI message")
...
class TestDashboardMetricsCalculations:
    ...
    def test_session_duration_calculation(self, team, experiment, participant):
        ...
        session = ExperimentSession.objects.create(
            experiment=experiment, participant=participant, team=team, created_at=start_time, ended_at=end_time
        )
    ...
    def test_completion_rate_calculation(self, team, experiment, participant):
        ...
        completed_session = ExperimentSession.objects.create(
            experiment=experiment, participant=participant, team=team, ended_at=timezone.now()
        )

        # Create incomplete session (no end time)
        incomplete_session = ExperimentSession.objects.create(experiment=experiment, participant=participant, team=team)
    ...
    def test_messages_per_session_calculation(self, team, experiment, participant):
        ...
        # Create 5 messages
        for i in range(5):
            ChatMessage.objects.create(chat=chat, message_type=ChatMessageType.HUMAN, content=f"Test message {i}")
