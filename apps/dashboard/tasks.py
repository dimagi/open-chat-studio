...
def cleanup_old_snapshots(days_to_keep=365):
    ...
    deleted_count = DashboardMetricsSnapshot.objects.filter(date__lt=cutoff_date).delete()[0]

    logger.info(f"Cleaned up {deleted_count} old metrics snapshots (older than {cutoff_date})")

    return {"snapshots_deleted": deleted_count, "cutoff_date": cutoff_date.isoformat(), "days_kept": days_to_keep}
