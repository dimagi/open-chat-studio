...
from .models import DashboardCache, DashboardMetricsSnapshot
...
class DashboardService:
    ...
    def get_filtered_queryset_base(self, start_date: Optional[datetime] = None, 
        ...
        sessions = ExperimentSession.objects.filter(**base_filters)
    ...
    def get_active_participants_data(self, granularity: str = 'daily', **filters) -> List[Dict[str, Any]]:
        ...
        participant_stats = (
            ...
            .values('period')
            .annotate(
                active_participants=Count('chat__experiment_session__participant', distinct=True)
            )
            .order_by('period')
        )
    ...
    def get_session_analytics_data(self, granularity: str = 'daily', **filters) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for stat in session_stats:
            ...
            data['participants'].append({
                'date': period_str,
                'unique_participants': stat['unique_participants']
            })
    ...
    def get_message_volume_data(self, granularity: str = 'daily', **filters) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for stat in message_stats:
            period_str = stat['period'].isoformat() if hasattr(stat['period'], 'isoformat') else str(stat['period'])
            data['human_messages'].append({
                'date': period_str,
                'count': stat['human_messages']
            })
            data['ai_messages'].append({
                'date': period_str,
                'count': stat['ai_messages']
            })
            data['totals'].append({
                'date': period_str,
    ...
    def get_user_engagement_data(self, limit: int = 10, **filters) -> Dict[str, Any]:
        ...
        participant_stats = participants.annotate(
        ...
        ).filter(total_messages__gt=0).order_by('-total_messages')
        
        # Most active participants
        most_active = []
        for participant in participant_stats[:limit]:
            avg_messages_per_session = (
                participant.total_messages / participant.total_sessions 
                if participant.total_sessions > 0 else 0
            )
            most_active.append({
    ...
    def get_channel_breakdown_data(self, **filters) -> Dict[str, Any]:
        ...
        for channel in channels:
            ...
            channel_messages = querysets['messages'].filter(
        ...
        for item in channel_data:
            ...
            item['participant_percentage'] = (item['participants'] / total_participants * 100) if total_participants > 0 else 0
    ...
    def get_tag_analytics_data(self, **filters) -> Dict[str, Any]:
        ...
        data = {
            'tag_categories': tag_stats,
            'total_tagged_messages': tagged_messages.count()
        }
    ...
    def get_overview_stats(self, **filters) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'active_experiments': querysets['experiments'].filter(
                sessions__in=querysets['sessions']
            ).distinct().count(),
        ...
        stats['avg_sessions_per_participant'] = (
            stats['total_sessions'] / stats['active_participants']
            if stats['active_participants'] > 0 else 0
        )
