...
class DashboardMetricsSnapshot(models.Model):
    ...
    def generate_snapshot(cls, team, date=None):
        ...
        sessions = ExperimentSession.objects.filter(team=team, created_at__date=date)
        ...
        # Platform statistics - aggregate by platform instead of individual channels
        platforms_in_use = (
            ExperimentChannel.objects.filter(team=team, deleted=False).values_list("platform", flat=True).distinct()
        )

        channel_stats = {}
        for platform in platforms_in_use:
            platform_sessions = sessions.filter(experiment_channel__platform=platform)
            platform_messages = daily_messages.filter(chat__experiment_session__experiment_channel__platform=platform)
            ...
            channel_stats[platform] = {
                ...
                "participants": platform_participants.count(),
            }
        ...
        session_msg_counts = sessions.annotate(msg_count=Count("chat__messages")).aggregate(
        ...
        snapshot, created = cls.objects.update_or_create(
            ...
            defaults={
                ...
                "active_experiments": experiments.filter(sessions__created_at__date=date).distinct().count(),
                "active_participants": all_participants.filter(experimentsession__created_at__date=date)
                .distinct()
                .count(),
                "new_participants": all_participants.filter(created_at__date=date).count(),
                "new_sessions": sessions.count(),
                "messages_sent": daily_messages.count(),
                "human_messages": daily_messages.filter(message_type=ChatMessageType.HUMAN).count(),
                "ai_messages": daily_messages.filter(message_type=ChatMessageType.AI).count(),
