...
def test_options_describes_the_body_the_route_takes(client, chatbot, route, verb, fields):
    """One view serves both routes, so the body described has to be resolved per verb rather than per
    class -- and since PATCH is the detail route's only writable verb, stock DRF metadata (PUT and
    POST alone) would answer it with no body at all."""
...
def test_a_write_bumps_the_edit_revision(client, chatbot):
    """The UI builder refuses a save whose `base_revision` has moved on, so an API write that left
    the revision alone would let an open builder session overwrite it with no conflict shown."""
