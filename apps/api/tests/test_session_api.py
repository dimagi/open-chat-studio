...
@pytest.mark.django_db()
@pytest.mark.parametrize("endpoint", ["list", "detail"], ids=["list", "detail"])
def test_session_participant_data_uses_working_version_id_for_versioned_experiment(experiment, endpoint):
    """Participant data is stored against the working version, so a session on a published
    version of the experiment must resolve participant_data from the working version id."""
    version = experiment.create_new_version()
    session = ExperimentSessionFactory.create(experiment=version)
    participant_data_value = {"name": "Alice"}
