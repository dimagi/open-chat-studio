...
class TestUploadFilesToVectorStore:
    @patch("apps.assistants.sync.create_files_remote", side_effect=_create_files_remote_side_effect("ext-file-id"))
    def test_upload_files_task_success(self, create_files_remote, collection_file, index_manager_mock):
        ...
        index_manager_mock.add_files_to_index.return_value = ([collection_file.file], [])
        ...
        index_manager_mock.add_files_to_index.assert_called_once_with(
            [collection_file.file], chunk_size=1000, chunk_overlap=100, re_upload=False
        )

    def test_upload_files_task_failure(self, collection_file, index_manager_mock):
...
#         # The new vector store should already be created, but the files should still reference the old
...
#         index_manager_mock.add_files_to_index.assert_any_call(
