...
@pytest.mark.django_db()
class TestConsentBannerOobUpdate:
    """Saving settings via HTMX must include an OOB swap that refreshes the consent banner."""

    def _login_user_with_change_perm(self, client, team):
        user = team.members.first()
        user.user_permissions.add(Permission.objects.get(codename="change_experiment"))
        client.force_login(user)
        return user

    def _post_settings(self, client, team, experiment, overrides):
        """POST to the settings endpoint with sensible defaults, applying overrides on top."""
        url = reverse("chatbots:settings", args=[team.slug, experiment.id])
        data = {
    ...
    def test_oob_banner_cleared_after_adding_consent_form(self, client, team_with_users):
        """When a consent form is added and saved, the OOB response must remove the warning banner."""
        team = team_with_users
        user = self._login_user_with_change_perm(client, team)
        pipeline = Pipeline.objects.create(team=team, data={"nodes": [], "edges": []})
        consent_form = ConsentFormFactory.create(team=team)
        experiment = Experiment.objects.create(
            name="No Consent Bot", owner=user, team=team, pipeline=pipeline, consent_form=None
        ...
        response = self._post_settings(
            ...
            team,
            experiment,
            {"consent_form": consent_form.id, "conversational_consent_enabled": True},
        )
        content = response.content.decode()

        assert response.status_code == 200
        assert 'id="consent-banner"' in content
        assert "hx-swap-oob" in content
        assert "chatbot-consent-missing-banner" not in content
        assert "chatbot-consent-disabled-banner" not in content

    def test_oob_banner_updated_to_warning_after_removing_consent_form(self, client, team_with_users):
        """When a consent form is removed and saved, the OOB response must show the warning banner."""
        team = team_with_users
        user = self._login_user_with_change_perm(client, team)
        pipeline = Pipeline.objects.create(team=team, data={"nodes": [], "edges": []})
        consent_form = ConsentFormFactory.create(team=team)
        ...
        response = self._post_settings(
            ...
            team,
            experiment,
            {"consent_form": "", "conversational_consent_enabled": False},
        )
        content = response.content.decode()
