...
Facts that reviewers and agents repeatedly get wrong:

* PostgreSQL only (`config/db.py`, pgvector). Postgres-only ORM features are fine; portability to SQLite/MySQL is not a concern
* Dev tooling targets Linux/macOS. Windows compatibility is not a concern in `tasks.py`, `scripts/`, or the dev docker files
* Changelog and public docs live in a separate repo, dispatched on merge by `.github/workflows/docs-changelog-dispatch.yml` off the PR-template checkbox. A PR does not carry changelog files
* Generated — fix the source, not the file: `api-schemas/*.yml` (drf-spectacular, regenerated by `update-generated-files.yml`; add postprocessing hooks in `apps/api/schema.py`), `components/chat_widget/src/components/**/readme.md` (`stencil build --docs`, from the prop JSDoc)
* `Experiment`, `Assistant` and `Pipeline` default managers filter `is_archived=False` (`VersionsObjectManagerMixin`), and `is_editable` is `not is_archived` — a view loading one through `objects` already rejects non-editable records
* `apps/conftest.py` has an autouse fixture that unsets the current-team ContextVar before and after every test, so a `set_current_team()` call in a test cannot leak into another
* Tests must not depend on migration-seeded global rows (`LlmProviderModel`, `PricingRule`) — the suite runs with `--reuse-db`, which makes those tests order-dependent
