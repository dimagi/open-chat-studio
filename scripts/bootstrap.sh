    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -h|--help)
                ...
                echo "  - The --force flag is provided, OR"
                echo "  - The --check flag is provided (check mode only)"
                echo ""
                echo "Options:"
                ...
                echo ""
                echo "Examples:"
                echo "  ./bootstrap.sh --check          # Check prerequisites without installing"
...
# Check UV installation (read-only)
check_uv() {
...
check_python_deps() {
    ...
    if [ -f "pyproject.toml" ]; then
        info "pyproject.toml found"
        if [ -f ".venv/pyvenv.cfg" ] || [ -d ".venv" ]; then
            info "Virtual environment exists"
        else
            warn "Virtual environment not found. Run 'uv sync --frozen --dev' to install dependencies"
        fi
    else
...
main() {
    ...
    if [ "$FORCE_RUN" != true ] && [ "${CLAUDE_CODE_REMOTE:-false}" != "true" ]; then
        ...
        echo ""
        echo "To check prerequisites without installing:"
